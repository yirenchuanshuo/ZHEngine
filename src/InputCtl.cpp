...
class InputCtl::CameraAttributes {
public:
    glm::vec2 lastpos;
...
InputCtl::InputCtl():Camera(std::make_unique<CameraAttributes>())
{
}
...
{
    return Camera->View.view_matrix();
}

glm::mat4x4 InputCtl::get_projection_matrix()
{
    return Camera->View.projection_matrix();
}
...
void InputCtl::cursor_pos_callback(double xpos, double ypos)
{
    ...
    Camera->moving = true;
    auto delta = glm::fract((pos - Camera->lastpos) * 0.5f + 0.5f) * 2.0f - 1.0f;
    if (m_inputPref.orbit_binding.check_is_pressed(Camera->window)) {
        Camera->View.orbit(m_inputPref, delta, false);
    }
    else if (m_inputPref.drift_binding.check_is_pressed(Camera->window)) {
        Camera->View.orbit(m_inputPref, delta, true);
    }
    else if (m_inputPref.pan_binding.check_is_pressed(Camera->window)) {
        Camera->View.pan(m_inputPref, delta);
    }
    else if (m_inputPref.zoom_binding.check_is_pressed(Camera->window)) {
        Camera->View.zoom(m_inputPref, delta[m_inputPref.zoom_axis], false);
    }
    else if (m_inputPref.hitchcock_binding.check_is_pressed(Camera->window)) {
        Camera->View.zoom(m_inputPref, delta[m_inputPref.zoom_axis], true);
    }
    else {
        Camera->moving = false;
    }
    ...
    if (m_inputPref.clamp_cursor && !Camera->curclamped && (xpos >= width - 1 || ypos >= height - 1 || xpos <= 1 || ypos <= 1)) {
        ...
        glfwSetCursorPos(Camera->window, xpos, ypos);
        /* Camera->curclamped = true; */
    }
    else {
        /* Camera->curclamped = false; */
    }
}
...
{
    double xpos, ypos;
    glfwGetCursorPos(Camera->window, &xpos, &ypos);
    int width, height;
    glfwGetWindowSize(Camera->window, &width, &height);
    ...
    Camera->moving = m_inputPref.orbit_binding.check_is_pressed(Camera->window)
        || m_inputPref.drift_binding.check_is_pressed(Camera->window)
        || m_inputPref.pan_binding.check_is_pressed(Camera->window)
        || m_inputPref.zoom_binding.check_is_pressed(Camera->window)
        || m_inputPref.hitchcock_binding.check_is_pressed(Camera->window);

    GLFWcursor* cursor = glfwCreateStandardCursor(Camera->moving ? GLFW_CROSSHAIR_CURSOR : GLFW_ARROW_CURSOR);
    glfwSetCursor(Camera->window, cursor);
}
...
void InputCtl::scroll_callback(double xoffset, double yoffset)
{
    ...
    if (m_inputPref.orbit_binding.check_is_scrolled(Camera->window)) {
        Camera->View.orbit(m_inputPref, delta, false);
    }
    else if (m_inputPref.drift_binding.check_is_scrolled(Camera->window)) {
        Camera->View.orbit(m_inputPref, delta, true);
    }
    else if (m_inputPref.pan_binding.check_is_scrolled(Camera->window)) {
        Camera->View.pan(m_inputPref, delta);
    }
    else if (m_inputPref.zoom_binding.check_is_scrolled(Camera->window)) {
        Camera->View.zoom(m_inputPref, delta[m_inputPref.zoom_axis], false);
    }
    else if (m_inputPref.hitchcock_binding.check_is_scrolled(Camera->window)) {
        Camera->View.zoom(m_inputPref, delta[m_inputPref.zoom_axis], true);
    }
}
